...
# 本模块只在其上叠加「可控/不可控」派生层与「销售↔随访」跨表关联，不改 churn_logic 内部规则。
...
# ---------------- 可控/不可控 派生层（叠加在 churn_logic 细分类之上） ----------------
# 说明：churn_logic 把原因归到 10 个细类 + 一级(医生/患者/其他)，这是「归因维度」。
# 另叠加「可控性」维度，用于改进措施落责：药房/运营可干预 = 可控；临床/医嘱主导或客观 = 不可控。
#   不可控：1 医嘱改间隔 / 2 医嘱停药 / 3 医嘱换药 / 7 去世
#   可控  ：4 不良反应(副作用管理可干预) / 5 认知不足 / 6 经济负担 / 8 换渠道 / 9 联系失败
#   其他  ：10 其他（待定）
CONTROL_OF = {
    '1_医嘱改变用药时间间隔': '不可控',
    '2_医嘱停药': '不可控',
...
CONTROL_DISPLAY = {'可控': '可控(运营可干预)', '不可控': '不可控(临床/客观)', '其他/未分类': '其他/未分类'}

def control_of(code):
    if code is None or (isinstance(code, float) and pd.isna(code)):
        return None
    return CONTROL_OF.get(code, '其他/未分类')
...
def load_followup(paths, pharm_map=None):
    ...
    df['原因类'] = df['脱落原因分类代码'].map(control_of)            # 可控/不可控(派生)
    # ---- 用药周期状态 → 5 组 → 三分类 ----
...
def crossref_brand(sales_last, followup):
    """品牌层面关联：销售各品种脱落B人数 ↔ 随访各品种脱落原因构成（可控/不可控）。"""
...
def dropout_reason_distribution(followup):
    """脱落原因细分类分布：基于『有记载脱落原因』的随访记录（原因原始非空→已归类）。
    返回 (dist[细类], lvl1[一级汇总], ctrl[可控性汇总], meta[覆盖])。"""
    sub = followup[followup['脱落原因分类代码'].notna()].copy()
    total = len(sub)
    rows = []
    for code, name in cl.CATEGORY_DISPLAY.items():
        ...
        rows.append({'一级分类': cl.LEVEL1_OF[code], '脱落原因细类': name,
                     '记录数': cnt, '占比%': round(cnt / total * 100, 1) if total else 0,
                     '可控性': CONTROL_OF.get(code, '其他/未分类')})
    ...
    lvl1['占比%'] = (lvl1['记录数'] / total * 100).round(1) if total else 0
    ctrl = dist.groupby('可控性')['记录数'].sum().reset_index().sort_values('记录数', ascending=False)
    ctrl['占比%'] = (ctrl['记录数'] / total * 100).round(1) if total else 0
    total_records = len(followup)
    no_reason = int((followup['脱落原因分类代码'].isna()).sum())
    meta = {'随访总记录': total_records, '有原因记录': total, '无原因记录': no_reason,
            '无原因占比%': round(no_reason / total_records * 100, 1) if total_records else 0}
    return dist, lvl1, ctrl, meta

def crossref_patient(sales_last, followup, pharm_map=None):
    ...
    for name, g in fsub.groupby('患者姓名'):
        ...
        if len(gr) > 0:
            ...
            lr_reason = ''; lr_code = None; lr_name = None; lr_lvl = None; lr_ctrl = None
...
def run_analysis(sales, followup=None, max_k=12, mult=3, with_patient_crossref=True):
    ...
    if followup is not None and not followup.empty:
        ...
        # 脱落原因细分类分布（一级汇总 / 可控性汇总 / 覆盖）
        try:
            d_dist, d_lvl1, d_ctrl, d_meta = dropout_reason_distribution(followup)
            result['dropout_reason_detail'] = d_dist
            result['dropout_reason_lvl1'] = d_lvl1
            result['dropout_reason_ctrl'] = d_ctrl
            result['dropout_reason_meta'] = d_meta
        except Exception:
            result.pop('dropout_reason_detail', None)
        # 患者级双视角明细（随访侧：最新一次 vs 最近一次有原因）
        try:
            _fu_meta = {
            ...
            result['patient_level_detail'] = cl.patient_level_detail(followup, _fu_meta)
        except Exception:
        ...
        if with_patient_crossref:
            pm = load_pharm_map()
            try:
                cp = crossref_patient(last, followup, pm)
            ...
            if cp is not None and not cp.empty:
                ...
                if not matched.empty:
                    rc = matched.groupby(['药房', '品牌', '脱落原因分类', '一级分类', '原因类']).size().reset_index(name='脱落患者数')
    ...
    _safe_df('dot_decomposition', dot_decomposition, sales)
    _safe_df('new_patient_monthly', new_patient_monthly, sales)
    _safe_df('new_patient_pharmacy_decline', new_patient_pharmacy_decline, sales)
    _safe_df('old_patient_multi_box', old_patient_multi_box, sales)
    _safe_df('repurchase_decomposition', repurchase_decomposition, sales)
    _safe_df('hospital_dimension', hospital_dimension, sales)
    try:
    ...
    _safe_df('pharmacy_dimension', pharmacy_dimension, sales)
    try:
