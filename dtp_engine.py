...
# ============================ 5. DOT / 新患 / 复购 / 维度分析 ============================
def resolve_window(sales, preset='H1_2026'):
...
def dot_decomposition(sales, wstart, wend, wstart_prev=None, wend_prev=None):
    ...
    for b in BRANDS:
        ...
        d_all_prev = _dot_snap(sub, wstart_prev, wend_prev) if has_prev else np.nan
        d_old = _dot_snap(sub, wstart, wend, old)
        d_old_prev = _dot_snap(sub, wstart_prev, wend_prev, old_prev) if has_prev else np.nan
        d_new = _dot_snap(sub, wstart, wend, new)
        d_new_prev = _dot_snap(sub, wstart_prev, wend_prev, new_prev) if has_prev else np.nan
        d_ov = _dot_snap(sub, wstart, wend, overlap)
        d_ov_prev = _dot_snap(sub, wstart_prev, wend_prev, overlap) if has_prev else np.nan
...
def new_patient_pharmacy_decline(sales, wstart, wend, wstart_prev=None, wend_prev=None):
...
def old_patient_multi_box(sales, wstart=None, wend=None, wstart_prev=None, wend_prev=None):
...
def hospital_dimension(sales, wstart=None, wend=None, wstart_prev=None, wend_prev=None):
...
def doctor_dimension(sales, wstart=None, wend=None, wstart_prev=None, wend_prev=None):
...
def drill_doctor_for_hospital(sales, hosp_df, wstart=None, wend=None, wstart_prev=None, wend_prev=None):
...
# ============================ 编排 ============================
def run_analysis(sales, followup=None, max_k=12, mult=3, with_patient_crossref=True, preset='H1_2026'):
    result = {}
    # 窗口信息（统一时间窗）
    wstart, wend, wstart_prev, wend_prev = resolve_window(sales, preset)
    ...
    # 留存率等月度指标：按时间窗过滤销售（full 预设 = 全量）
    sales_win = sales.copy()
    ...
    try:
        last = compute_dropout_B(sales, mult, end=wend)
    ...
    _safe_df('dot_decomposition', dot_decomposition, sales, wstart, wend, wstart_prev, wend_prev)
    _safe_df('new_patient_monthly', new_patient_monthly, sales)
    _safe_df('new_patient_pharmacy_decline', new_patient_pharmacy_decline, sales, wstart, wend, wstart_prev, wend_prev)
    _safe_df('old_patient_multi_box', old_patient_multi_box, sales, wstart, wend, wstart_prev, wend_prev)
    _safe_df('repurchase_decomposition', repurchase_decomposition, sales, wstart, wend, wstart_prev, wend_prev)
    _safe_df('hospital_dimension', hospital_dimension, sales, wstart, wend, wstart_prev, wend_prev)
    try:
        doc_df, top1, top5, docwatch = doctor_dimension(sales, wstart, wend, wstart_prev, wend_prev)
    ...
    _safe_df('pharmacy_dimension', pharmacy_dimension, sales, wstart, wend, wstart_prev, wend_prev)
    try:
        result['drill_hospital_doctor'] = drill_doctor_for_hospital(sales, result['hospital_dimension'], wstart, wend, wstart_prev, wend_prev)
    except Exception:
