...
def new_patient_monthly(sales):
    """新患月度趋势（按首购品种、首购月）。
    口径(用户拍板)：按药房算——患者在该药房第一次购药即该药房新患。
    故基准为 (患者ID, 药房名称) 首次购药，归属到该次首购的品种。"""
    sp = sales.sort_values('销售时间')
    fp = sp.groupby(['患者ID', '药房名称']).agg(首购月=('ym', 'first'), 首购品种=('品牌', 'first'))
    out = pd.DataFrame()
    for b in BRANDS:
        s = fp[fp['首购品种'] == b]
...
def new_patient_pharmacy_decline(sales, scope, wstart, wend, wstart_prev=None, wend_prev=None):
    ...
    口径(用户拍板)：按药房算——患者在该药房第一次购药即该药房新患。
    基准 = (患者ID, 药房名称) 首次购药月；同一患者在本药房买多个品种时，
    对其购买的每个品种都计为该药房新患（收购视角：该药房新增了这位患者）。"""
    if wstart is None:
        wstart = sales['销售时间'].min()
    if wend is None:
        wend = sales['销售时间'].max()
    has_prev = wstart_prev is not None and wend_prev is not None
    wp, we = wstart.to_period('M'), wend.to_period('M')
    wp_prev, we_prev = (wstart_prev.to_period('M'), wend_prev.to_period('M')) if has_prev else (None, None)
    # 患者在各药房的首购月（全量计算，天然处理跨药房转入）
    fp = sales.groupby(['患者ID', '药房名称'])['ym'].min().rename('首购月_药房').reset_index()
    pb = sales[['患者ID', '药房名称', '品牌']].drop_duplicates()
    m = pb.merge(fp, on=['患者ID', '药房名称'])
    rows = []
    for b in BRANDS:
        sub_b = m[m['品牌'] == b]
        for ph, g in sub_b.groupby('药房名称'):
            if scope == '项目' and '角色' in sales.columns and role_of(ph, b, sales) != '项目':
                continue
            n_now = int(((g['首购月_药房'] >= wp) & (g['首购月_药房'] <= we)).sum())
            n_prev = int(((g['首购月_药房'] >= wp_prev) & (g['首购月_药房'] <= we_prev)).sum()) if has_prev else 0
            if (not has_prev) and n_now == 0:
                continue
            if has_prev and n_prev == 0 and n_now == 0:
                continue
            city = sales[sales['药房名称'] == ph]['城市'].dropna().iloc[0] if '城市' in sales.columns and not sales[sales['药房名称'] == ph]['城市'].dropna().empty else ''
            rows.append({'品种': b, '药房': ph, '城市': city,
...
def _newp_ratio_unit(fm_unit, sub, unit_col, months):
    """按单位(药房/医院)口径的新患占比：fm_unit 为 (患者ID, 单位) 索引的首购月(period) Series；
    sub 为某一单位内的销售行；返回该单位患者中「首购落在本窗口月份」的比例。
    即：患者在该单位第一次购药，才算该单位的新患。"""
    if sub.empty:
        return np.nan
    unit_val = sub[unit_col].iloc[0]
    ids = [i for i in sub['患者ID'].unique() if pd.notna(i)]
    if not ids:
        return np.nan
    idx = [(i, unit_val) for i in ids]
    present = [k for k in idx if k in fm_unit.index]
    if not present:
        return np.nan
    return fm_unit.loc[present].isin(set(months)).mean()
...
def hospital_dimension(sales, scope, wstart=None, wend=None, wstart_prev=None, wend_prev=None):
    ...
    fm_hosp = sales.groupby(['患者ID', '医疗单位'])['ym'].min()  # 按医院首购（口径统一）
    proj = sales[sales['角色'] == '项目'] if (scope == '项目' and '角色' in sales.columns) else sales
    rows = []
    for b in BRANDS:
        sub = proj[proj['品牌'] == b]
        ...
        for hosp, g in sub.groupby('医疗单位'):
            ...
            rows.append({
                ...
                '新患占比': round(_newp_ratio_unit(fm_hosp, g, '医疗单位', months), 3),
...
def doctor_dimension(sales, scope, wstart=None, wend=None, wstart_prev=None, wend_prev=None):
    ...
    def newp_ratio_pharmacy(r):
        ...
        fm_pharm = sales.groupby(['患者ID', '药房名称'])['ym'].min()
        return round(_newp_ratio_unit(fm_pharm, sub, '药房名称', list(pd.period_range(wstart, wend, freq='M'))), 3)
...
def pharmacy_dimension(sales, scope='项目', wstart=None, wend=None, wstart_prev=None, wend_prev=None):
    """项目药房维度：大店低 DOT 识别。"""
    if wstart is None or wend is None:
        wstart, wend, wstart_prev, wend_prev = resolve_window(sales, 'H1_2026')
    months = list(pd.period_range(wstart, wend, freq='M'))
    dot_s, dot_e = _dot_window(wend)  # DOT 固定测量窗：窗口终点前推1年
    fm_pharm = sales.groupby(['患者ID', '药房名称'])['ym'].min()  # 按药房首购（口径统一）
    proj = sales[sales['角色'] == '项目'] if (scope == '项目' and '角色' in sales.columns) else sales
    rows = []
    for b in BRANDS:
        sub = proj[proj['品牌'] == b]
        ...
        for ph, g in sub.groupby('药房名称'):
            ...
            rows.append({
                ...
                '复购率_本期': round(r26, 3) if pd.notna(r26) else np.nan,
                '脱落率A': round(1 - r26, 3) if pd.notna(r26) else np.nan,
                '新患占比': round(_newp_ratio_unit(fm_pharm, g, '药房名称', months), 3)
            })
    df = pd.DataFrame(rows)
    if df.empty:
        return df
