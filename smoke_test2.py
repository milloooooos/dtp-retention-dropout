...
print('\n随访 原因类(可控/不可控) 分布:')
print(followup['原因类'].value_counts(dropna=False).to_string())
print('\n随访 脱落原因分类代码 TOP:')
...
print('\n===== 可控性汇总 =====')
print(res.get('dropout_reason_ctrl').to_string(index=False) if res.get('dropout_reason_ctrl') is not None else 'EMPTY')
print('\n===== 覆盖率 meta =====')
...
if cp is not None and not cp.empty:
    ...
    print('患者级样例(5行):')
    cols = [c for c in ['患者姓名','品牌','药房','关联方式','最新一次原因(随访)','最新一次分类','脱落原因(随访)','脱落原因分类','一级分类','原因类'] if c in cp.columns]
