...
print('\n===== 分析范围 scope 校验：项目药房 vs 全部药房 取数应不同 =====')
r_pj = E.run_analysis(sales, followup, max_k=12, mult=3, with_patient_crossref=True, preset='H1_2026', scope='项目药房')
r_all = E.run_analysis(sales, followup, max_k=12, mult=3, with_patient_crossref=True, preset='H1_2026', scope='全部药房')
db_pj = r_pj.get('dropout_B_by_brand')
db_all = r_all.get('dropout_B_by_brand')
print('脱落率B 分品种 项目药房:')
print(db_pj.to_string(index=False) if db_pj is not None and not db_pj.empty else 'EMPTY')
print('脱落率B 分品种 全部药房:')
print(db_all.to_string(index=False) if db_all is not None and not db_all.empty else 'EMPTY')
# 校验：项目药房患者总数 <= 全部药房患者总数（项目是全集子集）
tot_pj = db_pj['患者总数'].sum() if db_pj is not None and not db_pj.empty else 0
tot_all = db_all['患者总数'].sum() if db_all is not None and not db_all.empty else 0
print(f'=> 项目药房总患者数={tot_pj}, 全部药房总患者数={tot_all}')
...
# 医生维度记录数：项目应 <= 全部
dn_pj = len(r_pj.get('doctor_all') or []); dn_all = len(r_all.get('doctor_all') or [])
print(f'=> 医生维度记录数 项目={dn_pj}, 全部={dn_all}')
assert dn_pj <= dn_all, 'scope校验失败: 项目药房医生记录 > 全部药房!'
print('=> 分析范围 scope 校验通过(项目药房=全集子集, 维度表随范围变化)')
